...
class HomePage(QWidget):
    ...
    listStudyRequested = Signal(str, object)
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.star_combo.currentIndexChanged.connect(self._update_start_state)
    ...
    def _update_start_state(self, *_args: object) -> None:
        ...
        self.start_button.setEnabled(self.selected_list_key() is not None and available > 0)

    def _emit_list_study(self) -> None:
        key = self.selected_list_key()
        if key is not None and self.start_button.isEnabled():
            self.listStudyRequested.emit(key, self.star_combo.currentData())
