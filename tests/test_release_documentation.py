...
def test_readme_documents_the_complete_offline_dictionary_sources_and_counts():
    ...
    assert "33,433 个独立义项" in readme
    assert "14,214 个义项" in readme
    assert "356 个目标未收录" in readme
    assert "20,654 条 WordNet 3.0 语料例句" in readme
    assert "17,779 条明确标注的释义语境提示" in readme
